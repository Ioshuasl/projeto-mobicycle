import { db, generateId, hashPassword, verifyPassword, ensureSetting } from './db.ts';
import logger from './logger.ts';
import { MatrixManager } from './matrixManager.ts';
...
export class UserManager {
  ...
  static async resetSystem() {
    try {
      ...
      // MySQL: Desabilita checagem de FK temporariamente
      await db.exec("SET FOREIGN_KEY_CHECKS = 0");
      try {
        const tables = [
          'matrix_positions', 'matrix_history', 'matrix_cycles', 
          'user_badges', 'badges', 'transactions', 'notifications', 
          'documents', 'vouchers', 'push_subscriptions', 'matrices',
          'users'
        ];
        ...
        // Re-insert default settings only if they don't exist
        ...
        await db.prepare(`
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(adminId, "Admin Master", "Master", adminEmail, adminPassword, adminCpf, "00000000000", "MASTER", "admin", adminReferralCode, 'LICENSED', 1, 'APPROVED');
        
        await db.prepare(`
          INSERT INTO users (id, name, nickname, email, password, cpf, phone, status, role, referral_code, career_level, is_activated, document_status)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run("sys_admin_002", "Admin Mobicyclo", "Admin", "admin@mobicyclo.com", await hashPassword("admin"), "111.111.111-11", "11111111111", "MASTER", "admin", "MOBI001", 'LICENSED', 1, 'APPROVED');

        await db.prepare(`
          INSERT INTO users (id, name, nickname, email, password, cpf, phone, status, role, referral_code, career_level, is_activated, document_status)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run("sys_explosion", "Mobicyclo Explosion", "Explosion", "explosion@mobicyclo.com", await hashPassword("explosion"), "222.222.222-22", "22222222222", "MASTER", "admin", "EXPLOSION001", 'LICENSED', 1, 'APPROVED');
